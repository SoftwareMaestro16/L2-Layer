...
TON_L2_SKILLS = {
  ...
  l2_rollup_design: [
    ...
    "The only tracked public L1 deployment registry is deployments/testnet/entropis.json plus its schema; it may contain testnet addresses, code/data hashes, transaction hashes and getter evidence, but no wallet material, tokens, provider keys, private endpoints, DB/Redis URLs or signed BoCs.",
    ...
    "Operators can check public DA retrievability through GET /v1/operator/da/batch/{height}/{data_hash}; it runs the same verifier used before relayer signing and returns only safe status, static reason codes, public refs/URIs, payload size and latency.",
  ...
  sequencer_logic: [
    ...
    "L2 events are deterministic typed receipt data, currently ContractDeployed, ContractCalled, WithdrawalCreated, and FeeDistributed. They are committed through receipt_leaf_hash, receipt_root, and batch DA; a separate event_root requires a future block-header and L1 RollupRoot schema upgrade.",
    ...
    "L2 fee accounting is deterministic Phase A economics: charged gas/rejection fees are credited to configured sequencer, operator, and treasury L2 accounts, emit FeeDistributed receipt events, and use basis-point splits where rounding dust goes to the sequencer reward account.",
    "The Rust staking module is Phase B groundwork: stake/delegate/undelegate/unbond and rewards are deterministic, unbonding is block-height based, duplicate reward ids fail closed, and rewards must debit a payer account before they can be claimed.",
    "Observer replay must use the same FeeAccountingConfig as the sequencer because fee destination balances and FeeDistributed events affect state_root, receipt_root, and canonical DA hashes.",
    ...
    "The real TVM emulator boundary validates config BoC, libraries BoC, getter input/output stack BoCs, and action-list BoCs against size and single-root cell limits before trusting backend output or handing messages to the executor.",
    ...
    "TVM_ADAPTER=prototype is a local sample fallback only. It recognizes the sample L2 counter code cell and a restricted EnWallet V5 R1 no-action signed request path, then fails closed with tvm_adapter_not_implemented for unsupported code hashes.",
    "The EnWallet V5 prototype path uses the Acton-generated code hash and real data BoCs, verifies wallet_id, seqno, valid_until, and Ed25519 signatures, increments seqno deterministically, and rejects C5/extra actions until full TVM execution is hardened.",
  ...
  bridge_design: [
    ...
    "A live bridge E2E is green only after registry verification and a real testnet path completes DepositTon -> indexed L2 credit -> L2 transfer -> DA retrieval -> CommitBatch -> FinalizeBatch -> withdrawal proof -> ClaimWithdrawal -> AssetVault release with public-safe evidence.",

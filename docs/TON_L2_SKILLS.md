...
TON_L2_SKILLS = {
  ...
  sequencer_logic: [
    ...
    "Read-only contract getters are exposed through GET /v1/contract/{id}/state and POST /v1/contract/{id}/get-method. Getter execution snapshots state, validates method names/ids, optional stack BoC, gas and timeout limits, uses stored block timestamp context instead of wall clock for real TVM C7, and returns typed built-in results or a vm_stack_boc envelope without mutating the state root.",

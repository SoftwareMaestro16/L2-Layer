...
TON_L2_SKILLS = {
  ...
  sequencer_logic: [
    ...
    "The EnWallet V5 R1 seqno getter returns typed zero from source l2_uninitialized_wallet for missing or prefunded but undeployed wallet accounts. Other W5 getters still require a deployed W5 state cell so corrupted or non-wallet accounts are not masked.",
  ...
  infrastructure: [
    ...
    "EnWallet seed flow is BIP39 24 words -> PBKDF2-HMAC-SHA512 seed with salt mnemonic and 2048 iterations -> SHA256(entropis.enwallet.ed25519.seed.v1 || seed) -> Ed25519 keypair -> W5 init data cell -> L2 smart-contract account id from StateInit hash -> EX... address. Seed/private key must stay browser-local or wallet-local and never be sent to l2-node.",

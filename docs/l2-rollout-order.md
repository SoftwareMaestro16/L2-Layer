...
| 7 | EnWallet V5 build/test/deploy/call | Prove the canonical Tolk wallet can be built, deployed through `DeployContract`, called through `CallContract`, and read through getters. | `feat/enwallet-v5-l2-flow` | `feat(wallet): verify enwallet v5 l2 flow` | Acton checks, SDK EnWallet vectors, Rust deploy/call/getter tests, wallet UI integration smoke. |
...
| 9 | Staking and commission Phase A | Add deterministic fee accounting, visible operator/treasury/sequencer fee destinations, and Rust staking state-machine groundwork without touching L1 bridge custody. | `feat/l2-economics-fee-accounting` | `feat(l2): add deterministic fee accounting` | Fee split tests, overflow/rounding tests, receipt-root stability tests, staking state-machine tests, operator metrics/API tests. |
| 10 | Staking public transaction/API or TVM contract Phase B/C | Wire stake, delegate, undelegate, unbond, reward, commission into public L2 transaction/API surfaces, then migrate to TVM only after Phase A and TVM hardening are stable. | `feat/l2-staking-module` | `feat(l2): add deterministic staking module` | Public staking tx/API tests, early-withdrawal rejection, double-reward tests, deterministic replay, migration equivalence tests before TVM contract migration. |
| 11 | Sequencer decentralization groundwork | Add proposer identity/status modeling, leader-lock assumptions, liveness/censorship metrics, and future stake-backed selection without weakening the current single sequencer. | `feat/l2-proposer-set` | `feat(l2): add proposer set groundwork` | Proposer model tests, duplicate/out-of-order/unauthorized proposal tests, operator metrics tests, docs stating multi-proposer consensus is not active. |

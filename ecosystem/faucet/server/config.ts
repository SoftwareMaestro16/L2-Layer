...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): FaucetConfig {
  return {
    ...
    port: intEnv(env, "FAUCET_PORT", 3002),
...
function intEnv(env: NodeJS.ProcessEnv, key: string, fallback: number) {
...
function boolEnv(env: NodeJS.ProcessEnv, key: string, fallback: boolean) {

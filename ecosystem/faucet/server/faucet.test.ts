...
const USER: GitHubUser = { id: 42, login: "octo", avatarUrl: "https://example.test/a.png" };
...
test("github callback exchanges code and does not expose token in user", async () => {
  ...
  const user = await client.completeCallback({
    code: "code",
    redirectUri: "http://127.0.0.1:3003/api/auth/github/callback",
  });
...
test("node client falls back to single faucet endpoint on missing batch route", async () => {
  ...
  const client = new EntropisNodeClient(
    ...
    async (input) => {
      ...
      if (calls.length === 2) {
        return jsonResponse({ granted: true, deposit_id: "deposit-a" });
      }

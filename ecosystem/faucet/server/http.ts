...
export function redirect(response: ServerResponse, location: string, headers = {}): void {
...
export function requestOrigin(request: IncomingMessage): string {
  ...
  const host = request.headers["x-forwarded-host"] ?? request.headers.host ?? "127.0.0.1:3003";
...
export async function serveStatic(response: ServerResponse, pathname: string): Promise<boolean> {
